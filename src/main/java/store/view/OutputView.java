        ...
        for (Product product : products.getProducts()) {
            if (product.getPromotion() == null) {
                if (product.getStock().getNormalStock() == 0) {
                    System.out.printf(CURRENT_PRODUCT_NOT_STOCK, product.getName(), product.getPrice(), "재고 없음", "");
                    printMessage(BLANK);
                    continue;
                }
                System.out.printf(CURRENT_PRODUCT, product.getName(), product.getPrice(), product.getStock().getNormalStock(), "");
                printMessage(BLANK);
                continue;
            }
            if (product.getStock().getPromotionStock() == 0) {
                System.out.printf(CURRENT_PRODUCT_NOT_STOCK, product.getName(), product.getPrice(), "재고 없음", product.getPromotion().getName());
                printMessage(BLANK);
                continue;
            }
            System.out.printf(CURRENT_PRODUCT, product.getName(), product.getPrice(), product.getStock().getPromotionStock(), product.getPromotion().getName());
            printMessage(BLANK);
        }
    }
